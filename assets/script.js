  ...
  function setWeather(cityName) {
    ...
    axios.get(weatherURL).then(function (response) {
      ...
      CURRENTTEMPERATURE.innerHTML =
        "Temperature: " + kelvinFahrenheit(response.data.main.temp) + " &#176F";
      ...
      let forecastFiveDayURL =
        "https://api.openweathermap.org/data/2.5/forecast?id=" +
        cityID +
        "&appid=" +
        APIKEY;
      axios.get(forecastFiveDayURL).then(function (response) {
        ...
        for (i = 0; i < FORECASTCARDS.length; i++) {
          ...
          FORECASTTEMP.innerHTML =
            "Temp: " +
            kelvinFahrenheit(response.data.list[FORECASTINDEX].main.temp) +
            " &#176F";
